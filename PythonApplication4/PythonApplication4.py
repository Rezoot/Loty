...
def define_protobuf_classes():
    # Set up a dynamic descriptor for the Protobuf messages
    ...
    # Register and return message classes
    pool = descriptor_pool.DescriptorPool()
    pool.Add(file_desc)

    # Use GetMessages to obtain the message classes
    messages = message_factory.GetMessages([file_desc])
    Info = messages["flights.Info"]
    FlightData = messages["flights.FlightData"]
    Airport = messages["flights.Airport"]
    return Info, FlightData, Airport

def generate_tfs(departure_airport, arrival_airport, departure_date, return_date=None):
    Info, FlightData, Airport = define_protobuf_classes()
    info = Info()
    if return_date:
        info.seed_a = 1
        info.seed_b = -1
...
def generate_flights_url(tfs, lang="pl"):
...
if __name__ == "__main__":
    ...
    d=[]
    litera=list(map(chr, range(97, 123)))

    for k in litera:
        tekst="https://airportcodes.aero/iata/"+k
        r = requests.get(tekst)
        soup = BeautifulSoup(r.content, 'html.parser')

        test=soup.find_all("td", class_="")
    ...
    with open("porty.csv") as file:
        for f in file:
            ...
            tfs = generate_tfs("GDN", f[:3], date)
            url = generate_flights_url(tfs)
            print(url)
